...
public class BuddingIronBlock extends AmethystBlock {
    public static final int GROWTH_CHANCE = 5;
    private static final Direction[] DIRECTIONS = Direction.values();

    public BuddingIronBlock(Properties p_152726_) {
        super(p_152726_);
    }
    ...
    public void randomTick(BlockState p_220898_, ServerLevel p_220899_, BlockPos p_220900_, RandomSource p_220901_) {
        if (p_220901_.nextInt(5) == 0) {
            ...
            if (canClusterGrowAtState(blockstate)) {
                block = ModBlocks.SMALL_IRON_BUD.get();
            } else if (blockstate.is(ModBlocks.SMALL_IRON_BUD.get()) && blockstate.getValue(AmethystClusterBlock.FACING) == direction) {
                block = ModBlocks.MEDIUM_IRON_BUD.get();
            } else if (blockstate.is(ModBlocks.MEDIUM_IRON_BUD.get()) && blockstate.getValue(AmethystClusterBlock.FACING) == direction) {
                block = ModBlocks.LARGE_IRON_BUD.get();
            } else if (blockstate.is(ModBlocks.LARGE_IRON_BUD.get()) && blockstate.getValue(AmethystClusterBlock.FACING) == direction) {
                block = ModBlocks.IRON_CLUSTER.get();
            }
